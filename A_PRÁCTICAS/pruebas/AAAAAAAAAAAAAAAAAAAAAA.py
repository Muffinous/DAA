...
def checkeito(rows, columns, r, c):
    finalpos = c + rows[r] - 1
    for i in range(c, finalpos):
        if columns[i] == 0:
            return False
    return True
...
def isFeasible(sol, row_value, column_value, row, column, h_seen):
    null_values = row_value[row] != 0 and column_value[column] != 0  # there are still numbers
    #check = True
    #if row > 1:
        #check = checkeito(row_value, column_value, row, column)
    if not null_values:
        return False
    else:
...
def nonogram_solve(sol, rows, columns, r, c, h_seen):
    if allEmpty(rows, columns):
        ...
        while not is_sol and nojumps(sol, r, c, rows, columns) and rows[r] > 0 and max(columns[c:len(columns)]) <= len(rows) - r:
            ...
            if isFeasible(sol, rows, columns, r, c, h_seen):
                sol[r][c] = '#'
                h_seen.append(True)
                rows[r] -= 1
                columns[c] -= 1
                [sol, is_sol] = nonogram_solve(sol, rows, columns, r, c + 1, h_seen)
                if not is_sol:
                    sol[r][c] = '-'
                    rows[r] += 1
                    columns[c] += 1
            ...
            c += 1
        if isFullRow(rows[r]):
            [sol, is_sol] = nonogram_solve(sol, rows, columns, r + 1, 0, [False])
    return sol, is_sol
